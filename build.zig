...
pub fn build(b: *Builder) void {
    ...
    const exe = b.addExecutable(.{ .name = "bcrypt-encoder", .target = target, .root_source_file = b.path("main.zig"), .optimize = .ReleaseSafe, .version = .{ .major = version.major, .minor = version.minor, .patch = version.patch } });
